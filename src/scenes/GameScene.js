...
class GameScene extends Phaser.Scene {
    ...
    createPlanet(data, index) {
        ...
        if (data.type === 'safe') {
            if (this.level === 3) {
                this.add.text(data.x, data.y - data.radius - 28, '🚀 EMBARQUER !', {
                ...
                this.tweens.add({
                    targets: shipGlow, scaleX: 1.6, scaleY: 1.6, alpha: 0,
                    duration: 900, repeat: -1, ease: 'Sine.easeOut'
                });
            } else {
                this.add.text(data.x, data.y - data.radius - 28, '★ SAFE ?', {
                    fontSize: '20px', color: '#ffcc44', fontFamily: 'Arial Black'
                }).setOrigin(0.5);
            }
        }
        if (data.type === 'repulsive') {
            if (this.level === 2) {
                ...
                deco.setDisplaySize(w, h); deco.setDepth(20); deco.setOrigin(0.5, 1);
            }
        }
        ...
        this.tweens.add({
            ...
            onComplete: () => {
                ...
                this.time.delayedCall(500, () => {
                    ...
                    this.tweens.add({
                        targets: shipGfx,
                        y: shipData.y - 500, scaleX: 0.2, scaleY: 0.2, alpha: 0,
    ...
    triggerBlackHole(planetIndex) {
        ...
        this.tweens.add({
            ...
            onUpdate: (tween) => {
                ...
                if (p < 0.5) {
                    const t = p * 2;
                    gfx.setTint(Phaser.Display.Color.GetColor(Math.floor(120 * (1 - t)), 0, Math.floor(200 * (1 - t * 0.3))));
        ...
        this.time.delayedCall(1600, () => {
            ...
            const overlay = this.add.rectangle(
                ...
                this.scale.width, this.scale.height, 0x220044, 0
    ...
    showLevelBanner() {
        ...
        const banner = this.add.text(width / 2, height / 2, labels[this.level],
    ...
    showControls() {
        const msgs = {
            ...
            3: 'Dernier niveau — rejoins le vaisseau !'
        };
        const { width, height } = this.scale;
